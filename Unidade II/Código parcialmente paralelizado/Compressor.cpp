...
int compressor(int argc,char *argv[]){
    ...
    //preenche o vetor com 0 para evitar erros de acesso indevido na memória
    ...
    #pragma omp parallel for reduction(+:number[:256]) reduction(+:total_size, total_bits)
    for (int current_file = 1; current_file < argc; current_file++) {
        ...
        // Contar frequência dos bytes no nome do arquivo ou pasta
        ...
        if (this_is_not_a_folder(argv[current_file])) {
            ...
            if (local_original_fp) {
                ...
                for (long int j = 0; j < local_size; j++) {  
                    number[*local_x_p]++;
                    fread(local_x_p, 1, 1, local_original_fp);  // Leitura do próximo byte
                }
                fclose(local_original_fp);
            }
        } else {
            // Caso seja uma pasta, contar os bytes dentro dela
       ...
       // cout << "str_pointer: " << str_pointer << endl;
        while(*str_pointer){
            ...
            switch(*str_pointer){ 
                case '1':current_byte<<=1;current_byte|=1;current_bit_count++;break;
                case '0':current_byte<<=1;current_bit_count++;break;
                default:cout << "Ocorreu um erro" << endl << "O processo foi abortado";
                exit(2);
            }
            str_pointer++;
        }
