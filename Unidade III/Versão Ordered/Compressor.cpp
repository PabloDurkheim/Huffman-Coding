...
int compressor(int argc,char *argv[]){
    ...
    for(int current_file=1;current_file<argc;current_file++){
        ...
        if(this_is_not_a_folder(argv[current_file])){   //if current is a file and not a folder
            ...
            write_the_file_content(original_fp,size,str_arr,current_byte,current_bit_count,compressed_fp, ftell(compressed_fp));      //writes eighth
            }
            fclose(original_fp);
        }
...
void write_the_file_content(FILE *original_fp, long int size, string *str_arr, unsigned char &current_byte, int &current_bit_count, FILE *compressed_fp, int atualizar_escrita) {
    unsigned char *x_p, x;
    x_p = &x;
    char *str_pointer;
    ...
    #pragma omp parallel for private(posicao_escrita) firstprivate(buscar_bits, contagem_dos_bits, current_byte_teste, str_arr) \
    schedule(static, espacamento) ordered
    for(long int i = 0; i < size; i++){
              ...
              switch (*local_str_pointer) {
                ...
                if (contagem_dos_bits == 8) {
                         #pragma omp ordered
                        {
                          ...
                          contagem_dos_bits = 0;
                          //cout << "Sou a thread: " << omp_get_thread_num() << " | escrevi: " << bitset<8>(current_byte_teste) << " | posicao: " << posicao_escrita << endl;
                          posicao_escrita++;
                        }                                
                  }
                  local_str_pointer++;
