...
int descompressor(int argc,char *argv[]){
    ...
    //modifiquei aqui para acrescentar o .compressed no final
    int tamanho_do_char = strlen(argv[1]) + 12;
    
    char* novo_nome_arquivo = new char[tamanho_do_char];
    strcpy(novo_nome_arquivo, argv[1]);
    strcat(novo_nome_arquivo, ".compressed");
    ...
    fp_compressed=fopen(novo_nome_arquivo,"rb");
    if(!fp_compressed){
   ...
   // }
    ...
    //#pragma omp paralell for private(current_byte, current_bit_count, current_character, len, root)
    for(int i=0;i<letter_count;i++){
        ...
        //cout << "root: " << root << endl;
        //cout << "current_character: " << current_character << endl;
        //cout << "len" << len << endl;
        //int thread_id = omp_get_thread_num();
        //cout << "Thread: "<< thread_id << endl;
...
void translate_folder(string path,unsigned char &current_byte,int &current_bit_count,FILE *fp_compressed,translation *root){
    ...
    //#pragma omp parallel for private(new_path) shared(path, root, current_byte, current_bit_count, fp_compressed)
    for(int current_file=0;current_file<file_count;current_file++){
